...
import { getAttempt, setAttempt } from '../lib/redis';
...
export default (app: any) => {
  ...
  router.post('/login', async (req: UserAwareRequest, res: Response, next: NextFunction) => {
    try {
      const { email, password, client } = req.body;
      const response = await login(email, password, client);
      if (response) {
        return res.json({success: true, token: response});
      }
      const attempts = await getAttempt(email);
      if (attempts >= 3) {
  ...
  router.post('/refresh-token', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const response: any = await refreshToken(token, req.headers);
